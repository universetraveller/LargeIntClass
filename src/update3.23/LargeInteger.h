...
//多次构造类的地方可以inline
//3.22更新：
//增加了有异常处理机制的D2_A函数,由于都是标准库函数，不修改异常处理
//新增修改部分右值引用
// 优化部分构造函数，新增移动构造函数
//增加<<重载和copy constructor,优化ToString()
...
public:
	...
	LargeInteger(const LargeInteger& input) {
		is_minus_ = input.is_minus_;
		large_number_ = input.large_number_;
	}
	LargeInteger(deque<int>&& input, bool sign);
	LargeInteger();
